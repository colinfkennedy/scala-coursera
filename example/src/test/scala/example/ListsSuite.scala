...
class ListsSuite extends FunSuite {
  ...
  test("sum of some negative numbers") {
  ...
  test("sum of repeated negative numbers") {
  ...
  test("max of all negatives numbers") {
    assert(max(List(-3, -7, -2)) === -2)
  }

  test("max of all the same numbers") {
    assert(max(List(-3, -3, -3)) === -3)
  }

  test("max of two the same numbers") {
    assert(max(List(3, 2, 3)) === 3)
  }

  test("exception if empty list passed") {
